 ...
 * 
 * The message is base64-encoded and then split in parts with the maximum length
 * of a text message minus the header length for the first message.
 * 
 * The header consists of a identifier string, as defined in this class with
 * some metadata: - Parandroid Messaging protocol version (for backward
 * compatibility) - Compression type (0 is none): For future use (different
 * compression tables etc)
 */
public class MultipartDataMessage {
    ...
    private void setMessage(byte[] m) {
        String message = new String(Base64.encode(m, Base64.DEFAULT));
