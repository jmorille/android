 ...
 * </p>
 * <p>
 * {@link https://github.com/LarsWerkman/HoloColorPicker/}
 * </p>
 */
public class ColorPicker extends View {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mColorWheelRadius = min / 2 - mColorWheelThickness - (mColorPointerHaloRadius / 2);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			float[] pointerPosition = calculatePointerPosition(mAngle);
			if (x >= (pointerPosition[0] - mColorPointerHaloRadius) && x <= (pointerPosition[0] + mColorPointerHaloRadius) && y >= (pointerPosition[1] - mColorPointerHaloRadius)
					&& y <= (pointerPosition[1] + mColorPointerHaloRadius)) {
				mUserIsMovingPointer = true;
				invalidate();
			}
			// Check wheter the user pressed on the center.
			if (x >= -mColorCenterRadius && x <= mColorCenterRadius && y >= -mColorCenterRadius && y <= mColorCenterRadius) {
