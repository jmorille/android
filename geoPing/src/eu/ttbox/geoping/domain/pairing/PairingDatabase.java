...
public class PairingDatabase {
    ...
    public static class PairingColumns {
        ...
        COL_ID, COL_NAME, COL_PHONE, COL_PHONE_NORMALIZED, COL_PHONE_MIN_MATCH, COL_AUTHORIZE_TYPE, COL_SHOW_NOTIF, COL_PAIRING_TIME //
                , COL_NOTIF_SHUTDOWN, COL_NOTIF_BATTERY_LOW, COL_NOTIF_SIM_CHANGE, COL_NOTIF_PHONE_CALL, COL_NOTIF_PHONE_RECEIVE //
        };
    ...
    private boolean getSpyNotificationGlobalStatus(Context context, String notifColumnName) {
        ...
        Cursor c = context.getContentResolver().query(PairingProvider.Constants.CONTENT_URI, new String[] { notifColumnName }, null, null, null);
        while (c.moveToNext()) {
            boolean isActif = c.getInt(0) == 1 ? true : false;
            result |= isActif;
        }
        Log.i(TAG, "Get pyNotification GlobalStatus for " + notifColumnName + " : " + result);
        return result;
    }
