package eu.ttbox.geoping.test.osm.tiles;
...
import eu.ttbox.geoping.test.R;
...
public class MyAppTilesProvidersTest  extends AndroidTestCase {


	private final static String TAG = "MyAppTilesProvidersTest";
	...
	public void testSvg2Drawableandroid() {
		...
		InputStream is = testContext.getResources().openRawResource( R.raw.shapes); // getClass().getResourceAsStream(resName);
		SVG svg = SVGParser.getSVGFromInputStream(is);
		Drawable img = svg.createPictureDrawable();
		Log.d(TAG, "Open Drawable img : " + img);
	}
	...
	public void testSvg2Drawable() {
		Context testContext = getContext();
		InputStream is = testContext.getResources().openRawResource(  R.raw.tiles_svg_45100); // getClass().getResourceAsStream(resName);
		SVG svg = SVGParser.getSVGFromInputStream(is);
		Drawable img = svg.createPictureDrawable();
		Log.d(TAG, "Open Drawable img : " + img);
	}
