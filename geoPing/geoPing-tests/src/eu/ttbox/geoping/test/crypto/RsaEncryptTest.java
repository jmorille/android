...
public class RsaEncryptTest extends AndroidTestCase {
	...
	public RsaBytesEncryptor getBytesEncryptorFromSave() {
		...
		RsaBytesEncryptor bytesEncryptor = null;
		try {
			KeyFactory keyFactory = KeyFactory.getInstance("RSA");
			 
			PublicKey pubKey = keyFactory.generatePublic(new X509EncodedKeySpec(pubBytes));
			PrivateKey privKey = keyFactory.generatePrivate(new X509EncodedKeySpec(privBytes));
			bytesEncryptor = new RsaBytesEncryptor(RsaBytesEncryptor.RSA_ALGORITHM, pubKey, privKey);
		} catch (InvalidKeySpecException e) {
			throw new RuntimeException("InvalidKeySpecException : " + e.getMessage(), e);
