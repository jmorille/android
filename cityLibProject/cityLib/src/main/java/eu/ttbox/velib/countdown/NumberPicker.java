...
/**
 * This class has been pulled from the Android platform source code, its an internal widget that hasn't been made public so its included in the project in this
 * fashion for use with the preferences screen; I have made a few slight modifications to the code here, I simply put a MAX and MIN default in the code but
 * these values can still be set publically by calling code.
 * 
 * @author Google
 */
public class NumberPicker extends LinearLayout implements OnClickListener, OnFocusChangeListener, OnLongClickListener {
	...
	/*
	 * Use a custom NumberPicker formatting callback to use two-digit minutes strings like "01". Keeping a static formatter etc. is the most efficient way to do
	 * this; it avoids creating temporary objects on every call to format().
	 */
	...
	public NumberPicker(Context context, AttributeSet attrs, int defStyle) {
		...
		inflater.inflate(R.layout.countdown_number_picker, this, true);
	...
	/**
	 * Set the range of numbers allowed for the number picker. The current value will be automatically set to the start. Also provide a mapping for values used
	...
	protected void updateView() {
		...
		/*
		 * If we don't have displayed values then use the current number else find the correct value in the displayed values for the current number.
		 */
	...
	private class NumberRangeKeyListener extends NumberKeyListener {
		...
		public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
			...
			/*
			 * Ensure the user can't type in a value greater than the max allowed. We have to allow less than min as the user might want to delete some numbers
