...
public class BoundingBox implements Serializable {
	...
	public BoundingBox(double lat1, double lat2, double lng1, double lng2) {
		minLon = Math.min(lng1, lng2);
		maxLon = Math.max(lng1, lng2);
	...
	public boolean contains(WGS84Point point) {
		return (point.getLatitude() >= minLat) && (point.getLongitude() >= minLon) && (point.getLatitude() <= maxLat) && (point.getLongitude() <= maxLon);
	}
