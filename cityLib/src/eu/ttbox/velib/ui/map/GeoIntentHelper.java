...
public class GeoIntentHelper {
	...
	public static void startActivityGpsStatus(Context context) {
		try {
			...
			// gpsStatusIntent.setData(Uri.parse("market://details?id=com.eclipsim.gpsstatus"));
			gpsStatusIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//			gpsStatusIntent.setPackage("com.eclipsim.gpsstatus2");
			// gpsStatusIntent.setAction(Intent.ACTION_INSTALL_PACKAGE);
			...
			String marketPackage = "com.eclipsim.gpsstatus2";
			startActivityMarketDetails(context, marketPackage);
		}
	}
...
//		intent.setData(Uri.parse("market://search?q=pname:" + pkg));
		intent.setData(Uri.parse("market://details?id="+marketPackage));
		context.startActivity(intent);
	}
	
	public static void startActivityMarketSearch(Context context, String marketPackage) {
		Intent intent = new Intent(Intent.ACTION_VIEW);
		intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//		intent.setData(Uri.parse("market://search?q=pname:" + marketPackage)); 
		context.startActivity(intent);
	}
	...
	public static void startActivityStreetView(Context context, double lat, double lng) {
		...
		int zoom = 1;
		// The map zoom of the map location associated with this panorama. This value is passed on to the Maps activity when the Street View "Go to Maps"
		// menu item is chosen. It corresponds to the z parameter in the geo: intent.
		int mapZoom = 17;
