...
/**
 * @see http ://www.java2s.com/Open-Source/Android/UnTagged/veloid/com/xirgonium/ android /veloid/StationNearActivity.java.htm
 * @author jmorille
 * 
 */
public class StationDispoOverlay extends Overlay implements OnStationDispoUpdated, SharedPreferences.OnSharedPreferenceChangeListener {
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
		...
		// Other
		boundyBoxFixDelayInMs = AppConstants.PREFS_KEY_BOUNDYBOX_FIX_DELAY_IN_MS.equals(key) ? sharedPreferences.getLong(key,
				DEFAULT_PREF_BOUNDYBOX_FIX_DELAY_IN_MS) : boundyBoxFixDelayInMs;
		checkDispoDeltaDelayInMs = AppConstants.PREFS_KEY_CHEK_DISPO_DELTA_DELAY_IN_S.equals(key) ? sharedPreferences.getLong(key,
				DEFAULT_DISPO_DELTA_DELAY_IN_S) * ONE_SECOND_IN_MS : checkDispoDeltaDelayInMs;
		checkDispoBubbleDeltaDelayInMs = AppConstants.PREFS_KEY_CHEK_DISPO_BUBLE_DELTA_DELAY_IN_S.equals(key) ? sharedPreferences.getLong(key,
				DEFAULT_DISPO_BUBLE_DELTA_DELAY_IN_S) * ONE_SECOND_IN_MS : checkDispoBubbleDeltaDelayInMs;
		// Zoom Level
		zoomLevelDisplayStationDetail = AppConstants.PREFS_KEY_MIN_MAP_ZOOMLEVEL_DETAILS.equals(key) ? sharedPreferences.getInt(key,
				DEFAULt_KEY_MIN_MAP_ZOOMLEVEL_DETAILS)  : zoomLevelDisplayStationDetail;
	...
	public StationDispoOverlay(Context context, MapView mapView, ArrayList<Station> trackPoints, VelibService velibService, Handler handler,
		...
		this.checkDispoDeltaDelayInMs = sharedPreferences.getLong(AppConstants.PREFS_KEY_CHEK_DISPO_DELTA_DELAY_IN_S, DEFAULT_DISPO_DELTA_DELAY_IN_S)
				* ONE_SECOND_IN_MS;
		...
		if (!shadow) {
			if (selectedStation != null) {
				...
				if (balloonViewNotVisible) {
					...
					// Draw Layout
					balloonViewLayoutParams = new MapView.LayoutParams(MapView.LayoutParams.WRAP_CONTENT, MapView.LayoutParams.WRAP_CONTENT, point,
							MapView.LayoutParams.BOTTOM_CENTER, offsetX, offsetY);
					// balloonViewLayoutParams.alignment = MapView.LayoutParams.BOTTOM_CENTER;
					// balloonViewLayoutParams.mode = MapView.LayoutParams.MODE_MAP;
					// Visible
